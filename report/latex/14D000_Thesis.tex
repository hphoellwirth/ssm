...
With the explosion of computational power in the 1990s, particle filtering has become an established technique to solve state space models. It is commonly used for two types of inference problems: Inferring the latent state from observations, and estimating the state space model parameters. Established particle filter methods are limited with regards to their inference ability to particular state space models. The \textit{Kalman filter} method (Kalman, 1960) is restricted to linear Gaussian models for which the method uses the fact that the prediction and filtering densities can be solved analytically. For nonlinear models, these densities can only be approximated via sampling. The \textit{Sequential Importance Resampling (SIR)} algorithm by Gordon et al. (1993) in principle allows inference on both latent states and parameters for any kind of state space model. In practice, however, the method often produces poor parameter inference results due to its uneven log-likelihood curve with respect to the parameters. \textit{Continuous Sequential Importance Resampling (CSIR)} by Malik and Pitt (2011) fixes this problem but only works for univariate models. \\

For this reason, Brownlees and Kristensen (2017) proposed a new method called \textit{Importance Sampling Particle Filter} which allows to conduct parameter inference on any type of state space model. The main purpose of this project was to implement and test the new method alongside with established algorithms and to compare the method with these other filter algorithms.\\
...
\end{itemize}
In particular, none of those filters works (well) for parameter inference on nonlinear state space models.
