...
%The plots in \textit{Figure \ref{fig:ullm_loglik}} support the claim that the importance sampling particle filter is better suited for parameter inference than the SIR algorithm is.
%Note that the log-likelihood equals the log-likelihood of the particle filter if $\boldsymbol{\theta} = \boldsymbol{\tilde{\theta}}$ since all importance weights (both predictive and filtering) are $1$ in that case. What happens as the parameters get further apart? \\
